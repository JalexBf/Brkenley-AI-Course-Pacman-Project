...
def cornersHeuristic(state, problem):
    ...
    # Goal initial state is goal state
    if problem.isFinalState(problem.getInitialState()):
        return 0

    # Calculate distances from agent current position to all unvisited corners 
    for i in range(len(state[1])):
        if state[1][i] == 0:
            unvisitedCorners.append(corners[i])
            
    for corner in unvisitedCorners:
        distancesFromAgent.append(manhattanDistance(state[0], corner)) 

    # Calculate distances between unvisited corners
    for i in range(len(unvisitedCorners)):
        for j in range(i+1, len(unvisitedCorners)):
            distance = manhattanDistance(unvisitedCorners[i], unvisitedCorners[j])
            distanceBetweenCorners.append(distance)

    # Return the sum of the shortest path that covers all unvisited corners
    if len(unvisitedCorners) != 0:
        return min(distancesFromAgent) + sum(sorted(distanceBetweenCorners)[:len(unvisitedCorners)-1])
...
def foodHeuristic(state, problem):
    ...
    # Total remaining distance based on the closest dot
    remainingFood = foodGrid.count()
    
    return minDistance * remainingFood
